...
#ifndef SEC_ASSERT
// FIXME, port sec_assert library
#define SEC_ASSERT(t) \
    do { \
	if (!t) { \
	    printk("sec_assert: %s, %d\n", __FUNCTION__, __LINE__); \
...
static void mpc_cfg(void)
{
    ...
    uint32_t nspe_baddr = PART_SPE_NSPE_ADDR(nspe_partition);
    uint32_t nspe_laddr = nspe_baddr + DT_REG_SIZE(DT_NODELABEL(nspe_partition)) - 1;
    at_tz_mpc_config_region(nspe_baddr, nspe_laddr, AT_TZ_MPC_ATTR_NONSECURE);
...
#error CONFIG_ROM_START_OFFSET must be aligned to 2k boundary
    ...
    // allocate storage partition
    uint32_t storage_baddr = PART_ADDR(storage_partition);
    uint32_t storage_laddr = storage_baddr +
	DT_REG_SIZE(DT_NODELABEL(storage_partition)) - 1;
    at_tz_mpc_config_region(storage_baddr, storage_laddr,
	AT_TZ_MPC_ATTR_NONSECURE);

#if DT_NODE_EXISTS(DT_NODELABEL(slot1_partition))
    uint32_t slot1_baddr = PART_ADDR(slot1_partition);
    uint32_t slot1_laddr = slot1_baddr +
	DT_REG_SIZE(DT_NODELABEL(slot1_partition)) - 1;
    at_tz_mpc_config_region(slot1_baddr, slot1_laddr, AT_TZ_MPC_ATTR_NONSECURE);
#endif

#if DT_NODE_EXISTS(DT_NODELABEL(slot2_partition))
    uint32_t slot2_baddr = PART_ADDR(slot2_partition);
    uint32_t slot2_laddr = slot2_baddr +
	DT_REG_SIZE(DT_NODELABEL(slot2_partition)) - 1;
    at_tz_mpc_config_region(slot2_baddr, slot2_laddr, AT_TZ_MPC_ATTR_NONSECURE);
#endif

#if DT_NODE_EXISTS(DT_NODELABEL(fast_code_partition))
    uint32_t fast_code_baddr = PART_ADDR(fast_code_partition);
    uint32_t fast_code_laddr = fast_code_baddr +
	DT_REG_SIZE(DT_NODELABEL(fast_code_partition)) - 1;
    at_tz_mpc_config_region(fast_code_baddr, fast_code_laddr, AT_TZ_MPC_ATTR_NONSECURE);
#endif
...
static void sau_cfg(void)
{
    ...
    at_tz_sau_ret_t ret = at_tz_sau_enable_region(sau_region++,
...
#ifdef CONFIG_ATM_SPE_DISABLE_SHUB
    // lock out shub
