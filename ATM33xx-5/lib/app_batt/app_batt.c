...
#ifdef CONFIG_NO_BROWNOUT
#define NO_BROWNOUT CONFIG_NO_BROWNOUT
#endif
...
#else
static K_KERNEL_STACK_DEFINE(batt_timer_thread_stack, CONFIG_BATT_TIMER_STACK_SIZE);
static struct k_work_q batt_timer_work_queue;
static void batt_timer_expire(struct k_work *work);
static struct k_work_delayable batt_timer_work = Z_WORK_DELAYABLE_INITIALIZER(batt_timer_expire);
...
#else
	    ATM_LOG(E, "Invalid state, please check NO_BROWNOUT");
#endif // NO_BROWNOUT
	} break;
	case DEV_ACTV: {
#if (CFG_APP_BATT_FEATURE & APP_BATT_AUTO_TIMER_BIT)
#define QUICK_TIMER_CS 1
	    ...
	    k_work_reschedule_for_queue(&batt_timer_work_queue,
		&batt_timer_work, duration);
#endif // CONFIG_SOC_FAMILY_ATM
...
static void app_batt_sample_cb(uint8_t lvl)
{
	...
	k_work_reschedule_for_queue(&batt_timer_work_queue, &batt_timer_work,
	    duration);
#endif // CONFIG_SOC_FAMILY_ATM
#endif // CFG_APP_BATT_FEATURE & APP_BATT_AUTO_TIMER_BIT
    }
...
#else
    k_work_queue_init(&batt_timer_work_queue);
    k_work_queue_start(&batt_timer_work_queue, batt_timer_thread_stack,
	K_KERNEL_STACK_SIZEOF(batt_timer_thread_stack),
