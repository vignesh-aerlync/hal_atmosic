 ...
 * @file atm_hci_uart.c
 *
 * @brief ATM HCI data from uart to another device/CPU
...
LOG_MODULE_REGISTER(LOG_MODULE_NAME, CONFIG_ATM_LOG_DEFAULT_LEVEL);
    ...
    uint8_t const *data)
{
    ...
    uint8_t *hdlr_data = (uint8_t *)CONTEXT_VOID_P(data);
    hdlr->cmd_hdlr(hdlr_data);
    uint32_t size;
    uint8_t *bufptr;
...
static void tx_thread(void *p1, void *p2, void *p3)
{
	...
	struct net_buf *buf = net_buf_get(&tx_queue, K_FOREVER);
...
static void rx_thread(void *p1, void *p2, void *p3)
{
    ...
    for (;;) {
	struct net_buf *buf = net_buf_get(&rx_queue, K_FOREVER);
