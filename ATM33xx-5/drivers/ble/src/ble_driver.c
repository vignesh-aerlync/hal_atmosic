...
static int
ble_driver_send(struct net_buf *buf)
{
    LOG_DBG("enter");
...
static int ble_driver_open(void);

static struct bt_hci_driver const drv = {
    .name	= DT_INST_PROP(0, label),
    .bus	= BT_HCI_DRIVER_BUS_VIRTUAL,
    .open	= ble_driver_open,
    .send	= ble_driver_send,
};
#define BT_HCI_DRV_INFO (&drv)
#else
#define BT_HCI_DRV_INFO NULL
#endif // CONFIG_BT
...
#ifdef CONFIG_BT
    p_itf = rwtl_itf_get();
    bt_hci_driver_register(BT_HCI_DRV_INFO);
#else
    ble_driver_open();
#endif
    LOG_DBG("exit");
...
DEVICE_DT_INST_DEFINE(0, ble_driver_init, NULL, NULL, BT_HCI_DRV_INFO,
